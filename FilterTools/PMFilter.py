...
'''
	...
	而是将portfolio_user_id trader_weight放在PortfolioTraderWeightDbo，没有加入strategy作为key，而是作为trader的属性，
...
def is_satisfy(pm_item, filter_condition):
	...
	des = pm_item.describe(is_use_std=True, start_date=filter_start_date, end_date=filter_end_date,
...
def strategy_filter(df_strategy_info, df_trader_info, df_trader_pnl, list_filter_condition):
	...
	df_strategy_info = pd.DataFrame(
		df_strategy_info,
		columns=['Id', 'OutSampleDate', 'Type', 'OnlineDate']
	)
	df_trader_info = pd.DataFrame(
		df_trader_info,
	...
	for i in df_strategy_info.index:
		...
		obj_strategy = PMStrategy(Id=strategy_id, list_trader_id=list_traders_id)
	...
	print('Filtered All:    ', datetime.strftime(datetime.now(), '%H:%M:%S'))
...
def trader_filter(df_trader_info, df_trader_pnl, list_filter_condition):
	...
	for i_trader_id in list_trader_id:
		...
		obj_trader.set_trader_pnl(df_trader_pnl)
